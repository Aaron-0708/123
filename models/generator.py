...
class Generator(nn.Module):
    def __init__(self, device, zdim, in_channel, img_sz, out_channel):
        super(Generator, self).__init__()
        self.z_dim = zdim
        self.out_channel = out_channel
        self.device = device
        self.init_size = img_sz // 4
        self.l1 = nn.Sequential(nn.Linear(zdim, self.out_channel * self.init_size**2))#.to(self.device[0])
        self.conv_blocks0 = nn.Sequential(
            nn.BatchNorm2d(self.out_channel),
        )#.to(self.device[0])
        self.conv_blocks1 = nn.Sequential(
    ...
    def sample(self, size, device):
        # sample z
        z = torch.randn(size, self.z_dim).to(device[0])
        z = z.to(device[0])
        X = self.forward(z)
        return X
...
def CIFAR_GEN(device, bn=False, zdim=1000, in_channel=3, img_sz=224, out_channel=128, convdim=64):
    return Generator(device, zdim, in_channel, img_sz, out_channel)
