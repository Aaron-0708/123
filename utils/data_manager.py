...
class DataManager(object):
    ...
    def get_dataset(
        self, indices, source, mode, kshot=None, appendent=None, ret_data=False, m_rate=None
    ):
        if source == "train":
    ...
    def get_dataset_with_split(
        self, indices, source, mode, appendent=None, val_samples_per_class=0, kshot=None
    ):
        if source == "train":
            x, y = self._train_data, self._train_targets
        elif source == "test":
        ...
        if appendent is not None:
            ...
            for idx in range(0, int(np.max(appendent_targets)) + 1):
                append_data, append_targets = self._select(
                    appendent_data, appendent_targets, low_range=idx, high_range=idx + 1, kshot=kshot
                )
                val_indx = np.random.choice(
