...
class Teacher(nn.Module):
    def __init__(self, solver, generator, gen_opt, img_shape, iters, class_idx, deep_inv_params, train, args):
        ...
        self.mse_loss = nn.MSELoss(reduction="none").to(self._device[0])
        self.smoothing = Gaussiansmoothing(self._device[0], 3, 5, 1)
        if self.bn_loss:
    ...
    def sample(self, size, valid_out_dim, return_scores=False):
        self.solver.eval()
        self.generator.train()
        if self.first_time:
            self.first_time = False
            self.get_images(valid_out_dim, bs=size, epochs=self.iters, idx=-1)
        self.generator.eval()
        with torch.no_grad():
            x_i = self.generator.sample(size, self._device)
        # import pdb
        # pdb.set_trace()
        # with torch.no_grad():
        ...
        with torch.no_grad():
            outputs = self.solver(x_i, 1)
            logits = outputs["logits"][:, :valid_out_dim]
        _, y = torch.max(logits, dim=1)
        print(y)
        return (x_i, y, logits) if return_scores else (x_i, y)
    ...
    def get_images(self, valid_out_dim, bs=256, epochs=1000, idx=-1):
        torch.cuda.empty_cache()
        self.generator.train()
        self.generator.to(self._device[0])

        for epoch in tqdm(range(epochs)):
            # print('*'*10)
            inputs = self.generator.sample(bs, self._device)
            ...
            #     loss = ce_loss
            outputs = self.solver(inputs, 1)
            logits = outputs["logits"][:, :valid_out_dim]
            # logits = self._network(inputs, self.args["perturb_var"])["logits"]
            # logits[:, :self._known_classes] = float('-inf') 
            # loss = F.cross_entropy(logits, targets)
            # features = outputs["features"]
            loss = self.criterion(logits / self.content_temp, torch.argmax(logits, dim=1)) * self.content_weight
            if self.ie_loss:
                softmax_o_T = F.softmax(logits, dim=1).mean(dim=0)
                ie_loss = (1.0 + (softmax_o_T * torch.log(softmax_o_T) / math.log(valid_out_dim)).sum()) * self.w_ie
                loss += ie_loss
            # R_feature loss
